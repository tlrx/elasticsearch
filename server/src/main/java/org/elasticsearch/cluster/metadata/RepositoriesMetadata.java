    ...
    public static RepositoriesMetadata fromXContent(XContentParser parser) throws IOException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                final Set<SnapshotId> snapshots = new HashSet<>();
                while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                    if (token == XContentParser.Token.FIELD_NAME) {
                        ...
                        if ("uuid".equals(currentFieldName)) {
                            ...
                            while (parser.nextToken() != XContentParser.Token.END_ARRAY) {
                                SnapshotId snapshotToDelete = SnapshotId.parse(parser);
                                boolean added = snapshots.add(snapshotToDelete);
                                assert added : "found duplicate snapshot to delete: " + snapshotToDelete;
                            }
                        } else {
                ...
                repository.add(new RepositoryMetadata(name, uuid, type, settings, generation, pendingGeneration, List.copyOf(snapshots)));
            } else {
