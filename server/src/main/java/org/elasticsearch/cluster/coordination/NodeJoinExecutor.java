...
public class NodeJoinExecutor implements ClusterStateTaskExecutor<JoinTask> {
    ...
    public static void ensureIndexCompatibility(
        ...
        for (IndexMetadata idxMetadata : metadata) {
            ...
            if (idxMetadata.getCompatibilityVersion().before(minReadOnlySupportedVersion)) {
                throw new IllegalStateException(
                    "index "
                        + idxMetadata.getIndex()
                        + " version not supported: "
            ...
            if (idxMetadata.getCompatibilityVersion().before(minSupportedVersion)) {
                if (idxMetadata.getCompatibilityVersion().onOrAfter(minReadOnlySupportedVersion)) {
                    boolean isReadOnlyIndex = IndexMetadata.INDEX_BLOCKS_WRITE_SETTING.get(idxMetadata.getSettings());
                    if (isReadOnlyIndex) {
                        continue;
                    }
                    throw new IllegalStateException(
