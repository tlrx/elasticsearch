...
public class SyntheticIdTermsEnum extends BaseTermsEnum {
    ...
    private final NumericDocValues timestamps;  // sorted desc. order

    private SyntheticDocIdSetIterator iterator;

    public SyntheticIdTermsEnum(SortedDocValues  tsIdsDocValues, NumericDocValues timestampsDocValues) throws IOException {
    ...
    public SeekStatus seekCeil(BytesRef id) throws IOException {
        ...
        int lookupedTsIdOrd = iterator.lookupByTsId(tsIdTerm.get());
        if (lookupedTsIdOrd >= 0) {
            final long timestamp = ByteUtils.readLongBE(idAsBytes, 0);
            var docsIterator = createIterator(lookupedTsIdOrd, timestamp);
            this.iterator = docsIterator;
            return docsIterator.hasExactTimestamp() ? SeekStatus.FOUND : SeekStatus.NOT_FOUND;
    ...
    private SyntheticDocIdSetIterator createIterator(int tsIdOrdinal, @Nullable Long timestamp) throws IOException {
        ...
        if (0 <= tsIdOrdinal) {
            ...
            assert found : "Unknown _tsid ordinal: " + tsIdOrdinal;
    ...
    private static class SyntheticDocIdSetIterator extends DocIdSetIterator {
        ...
        private BytesRef syntheticId() throws IOException {
            ...
            if (timestamp != null) {
                ...
                return id(tsId, timestamps.longValue());
            }
        }
