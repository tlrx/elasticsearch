...
public class SyntheticIdTerms extends Terms {
    ...
    private static final SyntheticIdTerms EMPTY = new SyntheticIdTerms(DocValues.emptySorted(), DocValues.emptyNumeric(), 1);

    private final SortedDocValues tsIds;// sorted asc. order
    private final NumericDocValues timestamps;  // sorted desc. order
    private final int maxDocs;

    private SyntheticIdTerms(SortedDocValues tsIdsDocValues, NumericDocValues timestampsDocValues, int maxDocs) {
    ...
    public static SyntheticIdTerms from(SortedDocValues tsIds, SortedNumericDocValues timestamps, int maxDocs) {
        ...
        if (tsIds != null && timestamps != null) {
            return new SyntheticIdTerms(tsIds, DocValues.unwrapSingleton(timestamps), maxDocs);
        }
        return empty();
    }
