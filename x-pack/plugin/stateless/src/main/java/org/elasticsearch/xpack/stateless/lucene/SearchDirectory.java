...
public class SearchDirectory extends BlobStoreCacheDirectory {
    ...
    private void mergeMetadata(final Map<String, BlobFileRanges> incomingFileRanges, final boolean pitContextRelocationTransfer) {
        ...
        try {
            ...
            // Distinct BCC term/generations referenced by the generational files in this incoming metadata. A fresh commit
            // notification always references a single BCC, but PIT relocation metadata (see mergePITReaderMetadata) carries
            // generational files accumulated across many BCCs over the PIT's lifetime. We pin every one of them so that opening the
            // relocated commit (which re-opens these files and acquires each file's BCC term/generation, see
            // acquireGenerationalFileTermAndGeneration) always finds them present in generationalFilesTermAndGens. Re-pinning BCCs that
            // happen to be already held is intentional: it keeps them alive even if the only reader referencing them is closed
            // concurrently while we open the relocated commit.
            ...
            for (var entry : incomingFileRanges.entrySet()) {
                ...
                if (isGenerationalFile(fileName)) {
                    ...
                    // Pin the BCC of the ranges we actually keep (the first known location), which is what doOpenInput will use.
                    incomingGenerationalFilesTermAndGens.add(
                        reconciledMetadata.get(fileName).blobLocation().getBatchedCompoundCommitTermAndGeneration()
                    );
                } else {
