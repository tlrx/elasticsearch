...
public class CustomOkHttpGrpcExporter<T extends Marshaler> implements GrpcExporter<T> {
    ...
    /** Creates a new {@link CustomOkHttpGrpcExporter}. */
    public CustomOkHttpGrpcExporter(String type, OkHttpClient client, String endpoint, Headers headers, boolean compressionEnabled) {
        this.type = type;
        this.client = client;
    ...
    public CompletableResultCode export(T exportRequest, int numItems) {
        ...
        client.newCall(requestBuilder.build()).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                logger.log(
                    Level.SEVERE,
                    "Failed to export " + type + "s. The request could not be executed. Full error message: " + e.getMessage()
                );
                result.fail();
            }
            ...
            public void onResponse(Call call, Response response) {
                ...
                try {
                    ...
                    logger.log(Level.WARNING, "Failed to export " + type + "s, could not consume server response.", e);
                    failed.add(numItems);
                    result.fail();
                    return;
                }
                ...
                logger.log(
                    Level.WARNING,
                    "Failed to export " + type + "s. Server responded with " + codeMessage + ". Error message: " + errorMessage
                );
                result.fail();
            }
        });
