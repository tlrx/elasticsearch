...
public class SearchableSnapshotsIntegTests extends BaseSearchableSnapshotsIntegTestCase {
    ...
    public void testSnapshotMountedIndexLeavesBlobsUntouched() throws Exception {
        ...
        final SnapshotId snapshotOne = createSnapshot(repositoryName, "snapshot-1", singletonList(indexName)).snapshotId();
        assertAcked(client().admin().indices().prepareDelete(indexName));
        ...
        final SnapshotId snapshotTwo = createSnapshot(repositoryName, "snapshot-2", singletonList(indexName)).snapshotId();
    ...
    public void testSnapshotOfSearchableSnapshotIncludesNoDataButCanBeRestored() throws Exception {
        ...
        final SnapshotId snapshotTwo = createSnapshot(backupRepositoryName, "snapshot-2", singletonList(restoredIndexName)).snapshotId();
